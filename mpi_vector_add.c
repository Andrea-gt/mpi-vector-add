...
int main(void) {
   ...
   random_vector(local_x, local_n);  // For x vector
   random_vector(local_y, local_n);  // For y vector

   // Print random generated vectors
   Print_vector(local_x, local_n, n, "\nElements of x are:", my_rank, comm);
   Print_vector(local_y, local_n, n, "\nElements of y are:", my_rank, comm);

   // Perform the vector sum in parallel
